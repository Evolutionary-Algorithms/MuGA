...
/**
 * This class provides all necessary information and methods for the function
 * set in the CEC2013 Competition on Niching methods for multimodal Optimisation
 *
...
public final class CEC2013 {
    ...
    private static List< Func> funcs_ = Collections.unmodifiableList(new ArrayList< Func>(Arrays.asList(
            new F01_FiveUnevenPeakTrap(new BoundsFn.ConstantBoundsFn(1, new ClosedInterval.Double(0.0, 30.0))),
            new F02_EqualMaxima(new BoundsFn.ConstantBoundsFn(1, new ClosedInterval.Double(0.0, 1.0))),
            new F03_UnevenDecreasingMaxima(new BoundsFn.ConstantBoundsFn(1, new ClosedInterval.Double(0.0, 1.0))),
            new F04_Himmelblau(new BoundsFn.ConstantBoundsFn(2, new ClosedInterval.Double(-6.0, 6.0))),
            new F05_SixHumpCamelBack(new BoundsFn.ExplicitBoundsFn(
                    new ClosedInterval.Double(-1.9, 1.9),
                    new ClosedInterval.Double(-1.1, 1.1))),
            new F06_Shubert(new BoundsFn.ConstantBoundsFn(2, new ClosedInterval.Double(-10.0, 10.0))),
            new F07_Vincent(new BoundsFn.ConstantBoundsFn(2, new ClosedInterval.Double(0.25, 10.0))),
            new F06_Shubert(new BoundsFn.ConstantBoundsFn(3, new ClosedInterval.Double(-10.0, 10.0))),
            new F07_Vincent(new BoundsFn.ConstantBoundsFn(3, new ClosedInterval.Double(0.25, 10.0))),
            new F08_ModifiedRastriginAll(new BoundsFn.ConstantBoundsFn(2, new ClosedInterval.Double(0.0, 1.0))),
            new F09_CF1(new BoundsFn.ConstantBoundsFn(2, new ClosedInterval.Double(-5, 5))),
            new F10_CF2(new BoundsFn.ConstantBoundsFn(2, new ClosedInterval.Double(-5, 5))),
            new F11_CF3(new BoundsFn.ConstantBoundsFn(2, new ClosedInterval.Double(-5, 5))),
            new F11_CF3(new BoundsFn.ConstantBoundsFn(3, new ClosedInterval.Double(-5, 5))),
            new F12_CF4(new BoundsFn.ConstantBoundsFn(3, new ClosedInterval.Double(-5, 5))),
            new F11_CF3(new BoundsFn.ConstantBoundsFn(5, new ClosedInterval.Double(-5, 5))),
            new F12_CF4(new BoundsFn.ConstantBoundsFn(5, new ClosedInterval.Double(-5, 5))),
            new F11_CF3(new BoundsFn.ConstantBoundsFn(10, new ClosedInterval.Double(-5, 5))),
            new F12_CF4(new BoundsFn.ConstantBoundsFn(10, new ClosedInterval.Double(-5, 5))),
            new F12_CF4(new BoundsFn.ConstantBoundsFn(20, new ClosedInterval.Double(-5, 5)))
    )));
     ...
     *
     * @return a List of Func objects which corresponds to the function set of
     * the competition
     */
     ...
     * @return the fitness value of the global optima of the nfunc function
     * (1..20)
     */
    public static double getFitnessGoptima(final int nfunc) {
        return fopt_[nfunc - 1];
    }
	 ...
	 */
	public static int getDimension(final int nfunc) {
        return funcs_.get(nfunc - 1).getDimension();
    }
     ...
     */
    public static int getNoGoptima(final int nfunc) {
        return nopt_[nfunc - 1];
    }
     ...
     */
    public static double getRho(final int nfunc) {
        return rho_[nfunc - 1];
    }

    /**
     * Get the maximum number of function evaluations per function to be
     * optimised
     *
     * @param nfunc The number of function to be optimised (1..20)
     * @return the maximum number of function evaluations for the nfunc-th
     * function
     */
    public static long getMaxfes(final int nfunc) {
        return maxfes_[nfunc - 1];
    }

    /**
     * Calculate the seeds of a population (based on Algorithm 1 of the
     * technical report)
     *
     * @param pop the population of the potential solutions an ArrayList of
     * ArrayLists e.g. population with size NPxDim: An ArrayList with size NP
     * that contains Arraylists of "Dim" Doubles
    ...
    /**
     * This function calculates the number of global optima that exists inside a
     * population
     *
     * @param pop the population of the potential solutions
     * @param seeds the calculated seeds of the population
     * @param findex the index of the function that the user optimises BE
     * CAREFUL: the index should be in the range of (1...20)
     * @param accuracy the accuracy level for the experiment
     * @param radius the radius $\rho$
     * @return the number of global optima found within the pre-specified
     * accuracy
     */
    public static int howManyGlobalOptimaInPopulation(ArrayList<ArrayList<Double>> pop,
        ...
        for (ArrayList<Double> indi : seedstmp) {
            ...
            /* save time */
            if (count == CEC2013.getNoGoptima(findex)) {
                break;
            }
        }
