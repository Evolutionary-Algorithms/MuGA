...
public abstract class CFunc extends Func {
	...
	public void loadOptima(final String filename) 
	{
	...
	public void loadRotationMatrix(final String filename)
	{	
		File file = new File(filename); 
		try {
			LineNumberReader reader = new LineNumberReader( new FileReader( file ) );
...
//				        System.out.println("");
	...
	public void calculateWeights(final double [] x)
	{
...
//		System.out.println("");
	...
	public void initRotmatIdentity()
	{
		for (int i=0; i<nofunc_; ++i) {
//			System.out.println("Matrix: "+ i);
			for (int j=0; j<this.getDimension(); ++j) {
...
//			System.out.println("");
	...
	public void initOptimaRandomly()
	{
		...
		for (int i=0; i< nofunc_; ++i) {
			for (int j=0; j< this.getDimension(); ++j) {
				...
				O_[i][j] = bounds.get(j).getLower() + 
						(bounds.get(j).getUpper() - bounds.get(j).getLower()) * random.nextDouble();
				//	          System.out.print(O_[i][j] +"\t");
			}
			//	      System.out.println("");
		}
	}
	public void transformToZ(final double []x, final int index)
	{
	...
	public void transformToZNoshift(final double []x, final int index)
	{
		/* Calculate z_i = (x - o_i)/\lambda_i */
		for (int i=0; i<this.getDimension(); ++i) {
			//tmpx_[i] = (x[i] - O_[index][i])/lambda_[index];
...
//			System.out.println("FMAXI: "+index+" : "+fmaxi_[index]);
	...
	public double evaluateInner_(final double []x)
	{
	    double result = 0;
	    calculateWeights(x);
	    int index=0;
